...
export class Test2 implements AfterViewInit {
    ...
    map.on('load', () => {
      ...
      fetch('/assets/burkina_faso.geojson')
        ...
        .then(burkina => {
          ...
          // ── STRATÉGIE : on NE cache RIEN des couches OSM ─────────────────
          // Les autres pays (Niger, Mali, Ghana…) conservent toutes leurs
          // données OSM (labels, villes, routes…) intactes.
          //
          // Pour le Burkina, on ajoute NOS données PAR-DESSUS les couches
          // OSM. Grâce à l'ordre addLayer() et aux minzoom, nos données
          // prennent visuellement le dessus aux niveaux de zoom pertinents.
          // ─────────────────────────────────────────────────────────────────
          ...
          map.addLayer({
            ...
            source: 'burkina',
            paint: {
              // Léger fond pour distinguer visuellement le Burkina
              'fill-color': '#f5f0e8',
              'fill-opacity': 0.25
            }
          });
          ...
          map.addLayer({
            ...
            layout: {
              ...
              'text-optional': true,
              // 'symbol-sort-key' élevé = passe AU-DESSUS des labels OSM
              'symbol-sort-key': 10
            },
            paint: {
              'text-color': '#666666',
          ...
          // ── VILLES ────────────────────────────────────────────────────────
          // minzoom: 5 pour les cities → nos labels s'affichent bien avant
          // que les labels OSM soient visibles à ce niveau.
          ...
          map.addLayer({
            ...
            layout: {
              ...
              'text-anchor': 'top',
              'text-allow-overlap': false,
              'symbol-sort-key': 20   // priorité maximale
            },
            paint: {
              'text-color': '#111111',
              'text-halo-color': '#ffffff',
              ...
              new maplibregl.Popup({ maxWidth: '240px' })
                ...
                .setHTML(`
                  <div style="font-family:sans-serif;padding:4px">
                    ...
                    ${categorie
                      ? `<div style="color:#888;font-size:11px;margin-bottom:6px">${categorie}</div>`
                      : ''}
